...
<section class="guide-sec highlight"><h3>5 · Focus / spotlight <span class="tagimp">not obvious</span></h3><p><b>Target</b> tool on the left bar: <b>drag</b> to highlight an area — the rest dims. You can have <b>multiple focuses</b>. With the <b>Select</b> tool, touch a focus's <b>border</b> to <b>drag</b> it (move) and reveal the <b>✕</b> to remove it — the middle stays clickable for tokens. When a focus is selected: the top <b>handle</b> rotates it, the <b>shape</b> button toggles rectangle/ellipse and <b>✕</b> removes it. Toolbar buttons: <b>Zones</b> (👁) hides the areas and <b>Auto-focus</b> darkens the map and lights only the elements (PTs, players, objectives, enemies). <b>Ctrl+Z</b> undoes creation.</p></section>
...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  let focusLive = null, focusHidden = false, autoSpot = false, focusRotDrag = null;
  ...
  function renderFocus(previewRect) {
    ...
    if (!editable) { validManual.forEach(f => { const c = focusCenter(f); const g = new Konva.Group({ x: c.cx, y: c.cy, rotation: f.rot || 0 }); holeInto(g, f); focusLayer.add(g); }); }
      ...
      const active = focusRotDrag && focusRotDrag.idx === idx;
      ...
      // alça de rotação: nó separado (não entra no arraste de mover)
      const gap = Math.max(16, R * 1.2), ang = (f.rot || 0) * Math.PI / 180, dist = geo.hh / 2 + gap;
      const rhx = c.cx + Math.sin(ang) * dist, rhy = c.cy - Math.cos(ang) * dist;
      const rotG = new Konva.Group();
      rotG.add(new Konva.Line({ points: [c.cx, c.cy, rhx, rhy], stroke: 'rgba(240,198,107,0.45)', strokeWidth: 1.2, dash: [3, 3], listening: false, visible: active }));
      const rotHit = new Konva.Circle({ x: rhx, y: rhy, radius: br * 0.95, fill: '#f0c66b', stroke: '#0c0f16', strokeWidth: 1.2, visible: active });
      rotG.add(rotHit);
      let hideT = null;
      const showUI = () => { clearTimeout(hideT); delB.visible(true); shpB.visible(true); rotHit.visible(true); rotG.getChildren()[0].visible(true); focusLayer.batchDraw(); };
      const hideUI = () => { clearTimeout(hideT); hideT = setTimeout(() => { if (grp.isDragging() || focusRotDrag) return; delB.visible(false); shpB.visible(false); rotHit.visible(false); rotG.getChildren()[0].visible(false); focusLayer.batchDraw(); }, 280); };
  ...
  function wireEvents() {
    ...
    stage.on('mousemove touchmove', () => { if (focusRotDrag) doFocusRotate(); });
    ...
    stage.on('mouseup touchend', () => { if (focusRotDrag) { focusRotDrag = null; renderFocus(); saveProject(); } });

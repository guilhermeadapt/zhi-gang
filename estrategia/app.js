...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function refreshRoster() { renderGrid(); if (rosterView === 'board') renderBoard(); liveDraftTx(); }
  ...
  function importProjectFile(file) { const reader = new FileReader(); reader.onload = async () => {
    ...
    if (state.scenarios.some(s => !isPristine(s)) && !await askConfirm(t('confirmImport'))) return; applyImported(d);
  ...
  const LIVE = { peer: null, conns: [], host: false, code: '', myId: '', pen: null, on: false, applying: false, txT: null, follow: true, people: [], name: '', count: 0, viewT: null, lastView: '', canSave: false, pens: [], penLock: false, multi: false, draftT: null };
  ...
  function updateLiveUI() {
    ...
    const tx = $('liveBarTxt'); if (tx) { let s = (LIVE.host ? LIVE.code + ' · ' : '') + '👥 ' + (LIVE.count || 1); if (!mine) s += ' · ' + (LIVE.pens.length ? '✏️ ' + livePenNames() : t('livePenFreeS')); tx.textContent = s; }
    ...
    if (LIVE.on) {
      ...
      if (pw && pl) {
        ...
        if (LIVE.host) {
          ...
          h += LIVE.conns.map((c, i) => { const hasPen = LIVE.pens.indexOf(c.peer) > -1; return '<div class="lvp"><span class="lvp-n">' + (hasPen ? '✏️ ' : '') + esc(c._name || '—') + '</span><button class="lvp-t' + (hasPen ? ' on' : '') + '" data-pen="' + i + '">✏️ ' + t('givePen') + '</button><button class="lvp-t' + (c._canSave ? ' on' : '') + '" data-i="' + i + '">' + t('liveCanSave') + '</button><button class="lvp-t lvp-kick" data-kick="' + i + '" title="' + esc(t('kickBtn')) + '">⛔</button></div>'; }).join('');
          pl.innerHTML = h;
          ...
          pl.querySelectorAll('[data-kick]').forEach(b => b.addEventListener('click', () => { const c = LIVE.conns[+b.dataset.kick]; if (c) { try { c.close(); } catch (e) {} } }));
        }
      }
    }
  }

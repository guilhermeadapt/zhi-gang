...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function init() {
    ...
    renderSidebar(); renderRail(); loadScenarioIntoUI(); syncCanvasFill(cur()); fit(); applySceneView(cur()); updateZoomSaveBtn(); wireEvents(); (async () => { await maybeLoadShared(); await maybeLoadBoard(); await maybeLoadPlan(); await maybeLoadCommand(); autoSyncRoster(); })();
  ...
  const LIVE = { peer: null, conns: [], host: false, code: '', myId: '', pen: null, on: false, applying: false, txT: null, follow: true, people: [], name: '', count: 0, viewT: null, lastView: '' };
  ...
  function liveOnData(m, conn) {
    ...
    else if (m.t === 'hello' && LIVE.host) { conn._name = String(m.name || '—').slice(0, 20); toast('👥 ' + conn._name + ' ' + t('liveInName') + ' (' + (LIVE.conns.length + 1) + ')'); liveWho(); }
  ...
  function updateLiveUI() {
    ...
    if (!LIVE.on) { bar.hidden = true; document.body.classList.remove('live-locked'); return; }
  ...
  function fillLiveModal() {
    ...
    if (LIVE.on) { const inf = $('liveInfo'); if (inf) inf.textContent = LIVE.host ? (t('liveHosting') + ' ' + LIVE.code + ' · ' + LIVE.conns.length + ' ' + t('liveConnN')) : (t('liveGuestLbl') + ' ' + LIVE.code); const su = $('liveShareUrl'); if (su) su.value = liveShareLink(); }
  }

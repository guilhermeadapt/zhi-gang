...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
      ...
      const grupos = (CFG.grupos || []).map(g => ({
        ...
        pts: (g.pts || []).map(pid => {
          ...
          return {
            ...
            membros: ids.map(id => ({
              p: byId.get(id), tor: j.escalacao[id].torre, emote: specIcons ? specEmoteDe(byId.get(id)) : null,
      ...
      for (let i = 0; i < j.alt; i++) {
        ...
        j.grupos.forEach(g => g.pts.forEach((x, k) => {
          ...
          h += '<td' + (k === 0 ? ' class="pr-gsep' + (m && m.novo ? ' g-ch' : '') + '"' : (m && m.novo ? ' class="g-ch"' : ''))
            ...
            + (m ? (m.emote ? '<img class="g-ico" src="' + emoteUrl(m.emote) + '" alt="" loading="lazy">' : '') + '<span class="g-nome" style="border-left:3px solid ' + roleColor(m.p.funcao) + '">' + esc(nomeExib(m.p, m.emote ? 12 : 14)) + '</span>'
  ...
  async function printPNG() {
    ...
    jogos.forEach(j => {
      let cx = PAD + NW;
      g.fillStyle = '#161a23'; g.fillRect(PAD, y, NW + CW * cols, HH);
      g.fillStyle = '#d9a441'; g.font = 'bold 12px system-ui,sans-serif';
      g.fillText(String(j.nome).toUpperCase(), PAD + 6, y + HH / 2);
      j.grupos.forEach(gr => gr.pts.forEach((x, i) => {
        if (i === 0) { g.fillStyle = '#5a6274'; g.font = 'bold 8.5px system-ui,sans-serif'; g.fillText(String(gr.nome).toUpperCase(), cx + 7, y + 8); }
      ...
      y += HH;
      for (let i = 0; i < j.alt; i++) {
        g.fillStyle = '#5a6274'; g.font = '10px system-ui,sans-serif'; g.textAlign = 'right';
        g.fillText(String(i + 1), PAD + NW - 10, y + RH / 2); g.textAlign = 'left';
        cx = PAD + NW;
        j.grupos.forEach(gr => gr.pts.forEach(x => {
          const m = x.membros[i];
          if (m) {
            if (m.novo) { g.fillStyle = 'rgba(217,164,65,.14)'; g.fillRect(cx, y, CW, RH); }
            const im = m.emote ? imgs.get(m.emote) : null;
            if (im) { g.drawImage(im, cx + 4, y + 3, RH - 6, RH - 6); }
            else { g.fillStyle = roleColor(m.p.funcao); g.fillRect(cx + 4, y + 4, 2, RH - 8); }
            g.fillStyle = m.novo ? '#d9a441' : '#e8ecf4';
          ...
          } else { g.fillStyle = '#3d434f'; g.font = '11px system-ui,sans-serif'; g.fillText('—', cx + 11, y + RH / 2); }
          cx += CW;
        }));
    ...
    d.secs.forEach(s => {
      ...
      s.linhas.forEach(l => {
        ...
        l.cels.forEach((c, i) => {
          ...
          if (c && c.saiu) {
            g.fillStyle = 'rgba(217,164,65,.16)'; g.fillRect(x0, y, CW, RH);
            g.fillStyle = '#d9a441'; g.font = 'bold 12px system-ui,sans-serif';
          ...
          if (c && typeof c === 'object') {
            ...
            if (c.tor) {   // mesma marca do modo Planilha: letra fina, sem caixa
              g.fillStyle = { top: '#7fc6ff', mid: '#ffd166', bot: '#ff8fa3' }[c.tor];
  ...
  function bdChip(p) {
    ...
    return '<div class="bd-chip" draggable="true" data-id="' + p.id + '" style="--rc:' + roleColor(p.funcao) + '">'
      ...
      + '<span class="c-tags">' + tor + sabs + pin + tagsHtml + rep + bdFlagsHTML(p) + '</span>'

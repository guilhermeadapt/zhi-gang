...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function parseRaidHelper(signUps) { const out = []; signUps.forEach(s => { const cls = s.className || s.cClassName || ''; let status = 'primary', ausente = false, reserva = false, classe = cls, funcao = 'DPS'; if (/^absence$/i.test(cls)) { status = 'absence'; ausente = true; classe = 'Absence'; } else if (/^bench$/i.test(cls)) { status = 'bench'; reserva = true; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else if (/^late$/i.test(cls)) { status = 'late'; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else if (/^tentative$/i.test(cls)) { status = 'tentative'; reserva = true; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else { classe = cls; funcao = mapClass(cls); } out.push({ id: uid(), nome: s.name || '—', classe, funcao, status, ausente, reserva, pt: null, nota: s.note || '', spec: s.specName || s.cSpecName || undefined, specEmote: (s.specEmoteId && /^\d{5,20}$/.test(String(s.specEmoteId))) ? String(s.specEmoteId) : undefined, disc: (s.userId != null && /^\d{5,20}$/.test(String(s.userId))) ? String(s.userId) : undefined }); }); const rank = p => (p.ausente ? 3 : p.reserva ? 2 : 1), fo = f => ({ Tank: 0, Healer: 1, DPS: 2 }[f] ?? 3); out.sort((a, b) => rank(a) - rank(b) || fo(a.funcao) - fo(b.funcao) || a.nome.localeCompare(b.nome)); return out; }

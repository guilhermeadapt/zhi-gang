...
<section class="guide-sec highlight"><h3>5 · Focus / spotlight <span class="tagimp">not obvious</span></h3><p><b>Target</b> tool on the left bar: <b>drag</b> to highlight an area — the rest dims. You can have <b>multiple focuses</b>. With the <b>Select</b> tool, touch a focus's <b>border</b> to <b>drag</b> it (move) and reveal the <b>✕</b> to remove it — the middle stays clickable for tokens. The <b>Zones</b> (👁) button shows/hides all focus areas. <b>Ctrl+Z</b> undoes creation.</p></section>
...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  let focusLive = null, focusHidden = false;
  ...
  function renderFocus(previewRect) {
    ...
    // toggle de ocultar as zonas de foco (os dados ficam salvos, só não desenha)
    if (focusHidden && !previewRect) { focusLayer.batchDraw(); return; }
    const all = previewRect ? list.concat([previewRect]) : list;
    const valid = all.filter(f => f && f.w > 0.002 && f.h > 0.002);
    if (valid.length) {
      focusLayer.add(new Konva.Rect({ x: 0, y: 0, width: W, height: H, fill: 'rgba(6,8,12,0.62)', listening: false }));
      // buraco (clareia a área) + contorno dourado, por foco
      const holeBorder = (parent, f) => { const hx = f.x * W, hy = f.y * H, hw = f.w * W, hh = f.h * H, r = Math.min(hw, hh) * 0.05 + 6;
        parent.add(new Konva.Rect({ x: hx, y: hy, width: hw, height: hh, cornerRadius: r, fill: '#000', globalCompositeOperation: 'destination-out', listening: false }));
        parent.add(new Konva.Rect({ x: hx, y: hy, width: hw, height: hh, cornerRadius: r, stroke: 'rgba(240,198,107,0.55)', strokeWidth: 1.5, listening: false, shadowColor: '#f0c66b', shadowBlur: 14, shadowOpacity: 0.35 }));
        return { hx, hy, hw, hh, r }; };
      // interativo só na ferramenta Selecionar (como tokens/notas); nas outras
      // o foco aparece mas não captura clique (deixa desenhar/posicionar por baixo)
      const editable = !state.present && !previewRect && state.tool === 'select';
      if (!editable) { valid.forEach(f => holeBorder(focusLayer, f)); }
      else list.forEach((f, idx) => {
        if (!(f.w > 0.002 && f.h > 0.002)) return;
        // cada foco num grupo arrastável — buraco + contorno + moldura-alvo + ✕
        const grp = new Konva.Group({ draggable: true, name: 'focusg-' + idx });
        const geo = holeBorder(grp, f);
        // moldura-alvo: sem preenchimento (fillEnabled:false) → só a BORDA é alvo
        // (alargada por hitStrokeWidth); o interior passa o clique pros tokens.
        const frame = new Konva.Rect({ x: geo.hx, y: geo.hy, width: geo.hw, height: geo.hh, cornerRadius: geo.r, stroke: 'rgba(0,0,0,0.001)', strokeWidth: 2, fillEnabled: false, hitStrokeWidth: Math.max(20, R * 1.4) });
        grp.add(frame);
        const br = Math.max(8, R * 0.62), badge = new Konva.Group({ x: (f.x + f.w) * W, y: f.y * H, visible: false });
        badge.add(new Konva.Circle({ radius: br, fill: '#E25B52', stroke: '#2a0c0c', strokeWidth: 1.2, shadowColor: '#000', shadowBlur: 3, shadowOpacity: 0.6 }));
        badge.add(new Konva.Text({ text: '✕', fontFamily: 'Barlow, sans-serif', fontStyle: '700', fontSize: br * 1.1, fill: '#fff', align: 'center', verticalAlign: 'middle', width: br * 2, height: br * 2, offsetX: br, offsetY: br }));
        grp.add(badge);
        const showX = () => { badge.visible(true); focusLayer.batchDraw(); };
        const hideX = () => { if (!grp.isDragging()) { badge.visible(false); focusLayer.batchDraw(); } };
        badge.on('click tap', e => { e.cancelBubble = true; pushUndo(); focusList().splice(idx, 1); renderFocus(); saveProject(); });
  ...
  async function clearDrawings() { const hasM = (cur().marcas || []).length; if (!cur().desenhos.length && !hasM) return; if (!await askConfirm(t('confirmClearDraw'))) return; pushUndo(); cur().desenhos = []; cur().marcas = []; renderDrawings(); renderMarks(); renderTokens(); saveProject(); }

...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function mergeRoster(parsed) {
    ...
    parsed.forEach(np => { if (!curNames.has(norm(np.nome))) { if (np.dias) diaStatus(np, diaAtual()); result.push(np); added++; } });
    rosterDraft = result;
  ...
  async function fetchRoster() {
    ...
    [['sab', a], ['dom', b]].forEach(([d, lst]) => lst.forEach(p => {
      const k = norm(p.nome); let o = out.get(k);
  ...
  async function puxarExtras(link) {
    ...
    const norm = x => (x || '').trim().toLowerCase();
    const tem = new Set(rosterDraft.map(p => norm(p.nome)));
    const novos = lista.filter(p => p && p.nome && !tem.has(norm(p.nome)) && !/^absence$/i.test(p.classe || ''));
  ...
  function renderBoard() {
    ...
    const pool = byRole(active.filter(p => !p.pt && !p.reserva && !fora(p)));

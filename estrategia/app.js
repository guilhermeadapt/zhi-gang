...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function openBdPop(id, anchor) {
    const p = rosterDraft.find(x => x.id === id); if (!p || !bdPop) return; bdPopId = id; bdPopAnchor = anchor;
  ...
  function wireEvents() {
    ...
    document.querySelectorAll('.tema-sel').forEach(sel => {
      sel.innerHTML = TEMAS.map(x => '<option value="' + x + '">🎨 ' + esc(t('tema_' + x)) + '</option>').join('');
      sel.value = temaAtual;
      sel.addEventListener('change', () => {
        temaAtual = TEMAS.indexOf(sel.value) >= 0 ? sel.value : 'escuro';
        try { localStorage.setItem(TEMA_KEY, temaAtual); } catch (e) {}
        aplicarTema();
        document.querySelectorAll('.tema-sel').forEach(o => { o.value = temaAtual; });
        if (rosterModal && !rosterModal.hidden) refreshRoster();
        renderSidebar(); renderTokens();
        const gm = $('gradeModal'); if (gm && !gm.hidden) renderGrade();
      });
    });

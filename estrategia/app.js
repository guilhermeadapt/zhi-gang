  ...
  function gradeData() {
    ...
    if (gradeMode === 'sheet') {
        ...
        for (let i = 0; i < alt; i++) {
          linhas.push({
            ...
            cels: porJogo.map((col, j) => {
              ...
              return { txt: nomeCurto(p.nome), full: p.nome, cor: roleColor(p.funcao), novo: j > 0 && !antes };
            })
          });
        }
      ...
      PT_IDS.forEach(pid => {
        ...
        const linhas = ids.map(id => ({ label: nomeCurto(byId.get(id).nome), full: byId.get(id).nome, cor: roleColor(byId.get(id).funcao), cels: state.jogos.map(j => cel(j, id, pid)) }));
      ...
      const linhaDe = id => ({ label: nomeCurto(byId.get(id).nome), full: byId.get(id).nome, cor: roleColor(byId.get(id).funcao), cels: state.jogos.map(j => cel(j, id)) });
      PT_IDS.forEach(pid => {

...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  function openRoster() { rosterDraft = state.roster.map(p => Object.assign({}, p, { flags: (p.flags || []).slice() })); rosterPaste.value = ''; parseMsg.textContent = ''; parseMsg.className = 'parse-msg'; saveMsg.textContent = ''; renderGrid(); setRosterView(state.roster.length ? 'board' : 'grid'); rosterModal.hidden = false; }

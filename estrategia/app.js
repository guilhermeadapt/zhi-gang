...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function init() {
    ...
    renderSidebar(); renderJogos(); renderRail(); loadScenarioIntoUI(); syncCanvasFill(cur()); fit(); applySceneView(cur()); updateZoomSaveBtn(); wireEvents(); (async () => { await maybeLoadShared(); await maybeLoadScenes(); await maybeLoadObj(); await maybeLoadBoard(); await maybeLoadPlan(); await maybeLoadCommand(); await maybeAdoptRh(); autoSyncRoster(); maybeTour(); })();
  ...
  // Aceita os dois shapes da API: camelCase (v4, o que o bot serve) e o
  // lowercase antigo (/api/event/<id>). Normaliza antes de decidir qualquer coisa.
  function rhNorm(s) {
    return {
      name: s.name,
      note: s.note,
      className: s.className || s.cClassName || s.class || s.cClass || '',
      specName: s.specName || s.cSpecName || s.spec || s.cSpec || '',
      specEmoteId: s.specEmoteId || s.spec_emote,
      userId: s.userId != null ? s.userId : s.userid
    };
  }
  function parseRaidHelper(signUps) { const out = []; signUps.map(rhNorm).forEach(s => { const cls = s.className || s.cClassName || ''; let status = 'primary', ausente = false, reserva = false, classe = cls, funcao = 'DPS'; if (/^absence$/i.test(cls)) { status = 'absence'; ausente = true; classe = 'Absence'; } else if (/^bench$/i.test(cls)) { status = 'bench'; reserva = true; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else if (/^late$/i.test(cls)) { status = 'late'; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else if (/^tentative$/i.test(cls)) { status = 'tentative'; reserva = true; classe = s.specName || s.cSpecName || '—'; funcao = mapClass(classe); } else { classe = cls; funcao = mapClass(cls); } out.push({ id: uid(), nome: s.name || '—', classe, funcao, status, ausente, reserva, pt: null, nota: s.note || '', spec: s.specName || s.cSpecName || undefined, specEmote: (s.specEmoteId && /^\d{5,20}$/.test(String(s.specEmoteId))) ? String(s.specEmoteId) : undefined, disc: (s.userId != null && /^\d{5,20}$/.test(String(s.userId))) ? String(s.userId) : undefined }); }); const rank = p => (p.ausente ? 3 : p.reserva ? 2 : 1), fo = f => ({ Tank: 0, Healer: 1, DPS: 2 }[f] ?? 3); out.sort((a, b) => rank(a) - rank(b) || fo(a.funcao) - fo(b.funcao) || a.nome.localeCompare(b.nome)); return out; }
  ...
  // ---- integração com o bot de inscrições: o link do evento vira fonte viva ----
  // A API devolve o mesmo signUps do "Web view" e libera CORS, então dá pra
  // buscar direto do navegador. CFG.signupApis lista as bases tentadas em
  // ordem — é o que permite migrar do raid-helper para o bot próprio da guild
  // sem quebrar link antigo: quem não responder, passa a vez.
  function rhEventId(v) { const m = /(\d{15,20})/.exec(v || ''); return m ? m[1] : null; }
  const RH_BASES = (Array.isArray(CFG.signupApis) && CFG.signupApis.length)
    ? CFG.signupApis : ['https://raid-helper.xyz/api/v4/events/'];
  async function fetchRaidHelper(id) {
    for (const base of RH_BASES) {
      let d;
      try {
      ...
      const arr = Array.isArray(d.signUps) ? d.signUps : (Array.isArray(d.signups) ? d.signups : null);
      if (!arr) continue;
      const parsed = parseRaidHelper(arr);
      if (parsed.length) return parsed;
    }
    throw 0;
  }
  // O botão "Game Plan" no rodapé do evento abre o board com ?rh=<id>: adota o
  // evento como fonte do roster sem ninguém precisar colar link no modal.
  //
  // Trocar de evento é a parte perigosa. Existe UM projeto no navegador, então
  // abrir o link de um evento novo com o board do anterior montado mescla as
  // inscrições novas por cima das antigas: quem não se inscreveu no novo sai, e
  // a PT dessa pessoa vai junto. Fazer isso em silêncio é perder montagem sem
  // aviso, então pergunta. Só pergunta quando há o que perder: primeira adoção
  // ou board vazio entram direto, como antes.
  async function maybeAdoptRh() {
    const m = /[?&]rh=(\d{15,20})/.exec(location.search) || /[#&]rh=(\d{15,20})/.exec(location.hash);
    if (!m || state.rhEvent === m[1]) return;
    if (state.rhEvent && state.roster.length) {
      if (!(await askConfirm(t('rhSwitch'), t('rhSwitchYes'), t('rhSwitchNo')))) {
        // Fica no evento anterior. autoSyncRoster() segue e sincroniza AQUELE,
        // que é o que a pessoa acabou de pedir para manter.
        toast(t('rhSwitchKept'));
        return;
      }
    }
    state.rhEvent = m[1]; saveProject();
  }

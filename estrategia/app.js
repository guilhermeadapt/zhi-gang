...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function renderWarsMenu() {
    ...
    h += '<div class="pm-h">' + t('plansGuild') + '</div><div id="pmPlans"><div class="pm-empty">…</div></div>';
    menu.innerHTML = h;
    ...
    (async () => { let list = []; try { const res = await fetch('./plans/index.json', { cache: 'no-store' }); if (res.ok) list = await res.json(); } catch (e) {} const box = menu.querySelector('#pmPlans'); if (!box) return; if (!Array.isArray(list) || !list.length) { box.innerHTML = '<div class="pm-empty">' + t('plansEmpty') + '</div>'; return; } box.innerHTML = list.map(p => '<button class="pm-item" data-slug="' + esc(p.slug) + '"><span class="pm-name">' + esc(p.nome || p.slug) + '</span>' + (p.sub ? '<span class="pm-sub">' + esc(p.sub) + '</span>' : '') + '</button>').join(''); box.querySelectorAll('.pm-item').forEach(b => b.addEventListener('click', () => { menu.hidden = true; loadPlan(b.dataset.slug); })); })();
  }
  ...
  function livePeerOpts() { try { const o = JSON.parse(localStorage.getItem('zhi_live_server') || 'null'); if (o && o.host) return o; } catch (e) {} return undefined; }
  ...
  function liveReqPen() { if (!LIVE.on) return; if (LIVE.host) { liveSetPen(LIVE.myId); toast(t('livePenGot')); } else { const c = LIVE.conns[0]; if (c && c.open) c.send({ t: 'penReq', from: LIVE.myId }); } }
  function liveOnData(m, conn) {
    if (!m || !m.t) return;
    if (m.t === 'init' || m.t === 'state') {
      LIVE.gotInit = true;
      if (m.pen) LIVE.pen = m.pen;
    ...
    else if (m.t === 'hi' && LIVE.host) conn.send({ t: 'init', data: projectData(), pen: LIVE.pen });
  }
  function liveHost() {
    ...
    p.on('open', id => { LIVE.on = true; LIVE.myId = id; LIVE.pen = id; updateLiveUI(); fillLiveModal(); });
    p.on('connection', c => {
      c.on('open', () => { LIVE.conns.push(c); c.send({ t: 'init', data: projectData(), pen: LIVE.pen }); updateLiveUI(); fillLiveModal(); });
      c.on('data', m => liveOnData(m, c));
      c.on('close', () => { LIVE.conns = LIVE.conns.filter(x => x !== c); if (LIVE.pen === c.peer) liveSetPen(LIVE.myId); updateLiveUI(); fillLiveModal(); });
    });
  ...
  function liveJoin(code) {
    ...
    p.on('open', myId => {
      ...
      c.on('open', () => {
        ...
        LIVE.on = true; LIVE.gotInit = false; updateWarsBtn(); updateLiveUI(); fillLiveModal(); toast(t('liveJoined'));
  ...
  function liveTeardown() { LIVE.on = false; clearTimeout(LIVE.txT); LIVE.conns.forEach(c => { try { c.close(); } catch (e) {} }); LIVE.conns = []; try { if (LIVE.peer) LIVE.peer.destroy(); } catch (e) {} LIVE.peer = null; LIVE.host = false; LIVE.pen = null; LIVE.code = ''; updateLiveUI(); }
  ...
  function updateLiveUI() {
    ...
    const tx = $('liveBarTxt'); if (tx) tx.textContent = (LIVE.host ? LIVE.code + ' · ' : '') + (mine ? t('livePenYou') : t('livePenOther'));
    const pb = $('livePenBtn'); if (pb) pb.hidden = mine;
  }
  function liveShareLink() { return location.origin + location.pathname + '#c=' + LIVE.code; }
  function fillLiveModal() {
    const home = $('liveHome'), st = $('liveStatus'); if (!home || !st) return;
    home.hidden = LIVE.on || !!LIVE.peer; st.hidden = !LIVE.on;

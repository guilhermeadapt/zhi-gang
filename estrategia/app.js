        ...
        for (let i = 0; i < alt; i++) {
          linhas.push({
            ...
            cels: porJogo.map((col, j) => {
              ...
              return { txt: nomeExib(p), full: p.nome, cor: roleColor(p.funcao), novo: j > 0 && !antes };
            })
          });
        }
    ...
    d.secs.forEach(s => {
      ...
      s.linhas.forEach(l => {
        ...
        l.cels.forEach((c, i) => {
          ...
          if (c && typeof c === 'object') { h += '<td class="' + (c.novo ? 'g-ch' : '') + '" style="border-left:3px solid ' + c.cor + '" title="' + esc(c.full || c.txt) + '">' + esc(c.txt) + '</td>'; return; }

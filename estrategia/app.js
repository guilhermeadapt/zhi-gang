...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function init() {
    ...
    renderSidebar(); renderRail(); loadScenarioIntoUI(); syncCanvasFill(cur()); fit(); applySceneView(cur()); updateZoomSaveBtn(); wireEvents(); (async () => { await maybeLoadShared(); await maybeLoadScenes(); await maybeLoadObj(); await maybeLoadBoard(); await maybeLoadPlan(); await maybeLoadCommand(); autoSyncRoster(); })();
  ...
  async function maybeLoadObj() {
    ...
    renderObjectives(); renderTokens(); if (!objPanel.hidden) renderObjPanel(); updateSideChip(); saveProject(); toast(t('objLoaded'));
  }
  ...
  function newWar(nome) { const r = stashCurrentWar(); const id = 'w' + Date.now().toString(36); r.list.push({ id, nome: nome || ('Guerra ' + (r.list.length + 1)), ts: Date.now() }); r.cur = id; warsSave(r); applyImported(freshPlanData()); updateWarsBtn(); }
  function dupWar() { const r = stashCurrentWar(); const curW = r.list.find(w => w.id === r.cur); const id = 'w' + Date.now().toString(36); r.list.push({ id, nome: (curW ? curW.nome : 'Guerra') + ' (cópia)', ts: Date.now(), n: state.scenarios.length }); r.cur = id; warsSave(r); saveProject(); updateWarsBtn(); toast(t('warLoaded')); }
  ...
  const LIVE = { peer: null, conns: [], host: false, code: '', myId: '', pen: null, on: false, applying: false, txT: null, follow: true, people: [], name: '', count: 0, viewT: null, lastView: '', canSave: false };
  function liveGetName() { const el = $('liveNameIn'); let n = (el && el.value || '').trim().slice(0, 20); if (!n) { try { n = (localStorage.getItem('zhi_live_name') || '').slice(0, 20); } catch (e) {} } if (!n) n = 'Comandante'; try { localStorage.setItem('zhi_live_name', n); } catch (e) {} return n; }
  function livePenName() { const p = (LIVE.people || []).find(x => x.id === LIVE.pen); return p && p.name ? p.name : ''; }
  // host: recompila e transmite a lista de presentes (contador + nomes + modo seguir)
  function liveWho() { if (!LIVE.host) return; LIVE.count = LIVE.conns.length + 1; LIVE.people = [{ id: LIVE.myId, name: LIVE.name }].concat(LIVE.conns.map(c => ({ id: c.peer, name: c._name || '—' }))); const msg = { t: 'who', n: LIVE.count, people: LIVE.people, follow: LIVE.follow }; LIVE.conns.forEach(c => { if (c.open) c.send(msg); }); updateLiveUI(); }
  ...
  function livePenMine() { return !LIVE.on || LIVE.pen === LIVE.myId; }
  function liveViewTick() {
    if (!LIVE.on || !livePenMine() || !LIVE.follow) return;
    const sc = stage.scaleX() || 1;
    const v = { t: 'view', sid: state.currentId, scale: sc, cxf: clamp01((VW / 2 - stage.x()) / sc / W), cyf: clamp01((VH / 2 - stage.y()) / sc / H) };
    const key = v.sid + '|' + sc.toFixed(3) + '|' + v.cxf.toFixed(3) + '|' + v.cyf.toFixed(3);
    if (key === LIVE.lastView) return;
    LIVE.lastView = key;
    LIVE.conns.forEach(c => { if (c.open) c.send(v); });
  }
  function liveScheduleTx() { clearTimeout(LIVE.txT); LIVE.txT = setTimeout(() => { if (!LIVE.on || !livePenMine()) return; const msg = { t: 'state', data: projectData(), pen: LIVE.pen }; LIVE.conns.forEach(c => { if (c.open) c.send(msg); }); }, 250); }
  function liveRelay(m, from) { LIVE.conns.forEach(c => { if (c !== from && c.open) c.send(m); }); }
  function liveSetPen(id) { LIVE.pen = id; const msg = { t: 'pen', holder: id }; LIVE.conns.forEach(c => { if (c.open) c.send(msg); }); updateLiveUI(); }
  function liveReqPen() {
    if (!LIVE.on) return;
    if (livePenMine()) {   // largar: caneta fica livre pra qualquer um pegar
      if (LIVE.host) liveSetPen(''); else { const c = LIVE.conns[0]; if (c && c.open) c.send({ t: 'penDrop', from: LIVE.myId }); }
      return;
    }
    if (LIVE.host) { liveSetPen(LIVE.myId); toast(t('livePenGot')); }
    else { const c = LIVE.conns[0]; if (c && c.open) c.send({ t: 'penReq', from: LIVE.myId }); }
  }
  function liveOnData(m, conn) {
    if (!m || !m.t) return;
    if (m.t === 'init' || m.t === 'state') {
      LIVE.gotInit = true;
      if (m.pen !== undefined) LIVE.pen = m.pen;
      ...
      updateLiveUI();
    } else if (m.t === 'pen') { const was = livePenMine(); LIVE.pen = m.holder; if (LIVE.host) liveRelay(m, conn); if (!was && livePenMine()) toast(t('livePenGot')); updateLiveUI(); }
    else if (m.t === 'penReq' && LIVE.host) liveSetPen(m.from);
    else if (m.t === 'penDrop' && LIVE.host) { if (m.from === LIVE.pen) liveSetPen(''); }
    else if (m.t === 'perm') { const was = LIVE.canSave; LIVE.canSave = !!m.save; if (LIVE.canSave !== was) toast(LIVE.canSave ? t('livePermOn') : t('livePermOff')); updateLiveUI(); }
    ...
    else if (m.t === 'who') { const prev = LIVE.count; LIVE.count = m.n; if (Array.isArray(m.people)) LIVE.people = m.people; if (typeof m.follow === 'boolean') LIVE.follow = m.follow; if (!LIVE.host) { if (prev && m.n > prev) toast('👥 ' + t('liveJoinedN') + ' (' + m.n + ')'); else if (prev && m.n < prev) toast('👥 ' + t('liveLeftN') + ' (' + m.n + ')'); } updateLiveUI(); }
    else if (m.t === 'view') {
    ...
    else if (m.t === 'hi' && LIVE.host) conn.send({ t: 'init', data: projectData(), pen: LIVE.pen, n: LIVE.conns.length + 1, follow: LIVE.follow });
  }
  function liveHost() {
    ...
    p.on('open', id => { LIVE.on = true; LIVE.myId = id; LIVE.pen = id; LIVE.count = 1; LIVE.people = [{ id, name: LIVE.name }]; clearTimeout(LIVE.connT); LIVE.viewT = setInterval(liveViewTick, 400); updateLiveUI(); fillLiveModal(); });
    p.on('connection', c => {
      c.on('open', () => { LIVE.conns.push(c); LIVE.count = LIVE.conns.length + 1; c.send({ t: 'init', data: projectData(), pen: LIVE.pen, n: LIVE.count, follow: LIVE.follow }); updateLiveUI(); fillLiveModal(); });
      c.on('data', m => liveOnData(m, c));
      c.on('close', () => { LIVE.conns = LIVE.conns.filter(x => x !== c); if (LIVE.pen === c.peer) liveSetPen(LIVE.myId); toast('👥 ' + (c._name || '—') + ' ' + t('liveOutName') + ' (' + (LIVE.conns.length + 1) + ')'); liveWho(); fillLiveModal(); });
    });
  ...
  function liveJoin(code) {
    ...
    p.on('open', myId => {
      ...
      c.on('open', () => {
        ...
        r.list.push({ id, nome: '⚡ ' + LIVE.code, ts: Date.now(), live: true }); r.cur = id; warsSave(r);
  ...
  function liveTeardown() { LIVE.on = false; clearTimeout(LIVE.txT); clearTimeout(LIVE.connT); clearInterval(LIVE.viewT); LIVE.count = 0; LIVE.people = []; LIVE.follow = true; LIVE.canSave = false; try { setTool(state.tool); } catch (e) {} LIVE.conns.forEach(c => { try { c.close(); } catch (e) {} }); LIVE.conns = []; try { if (LIVE.peer) LIVE.peer.destroy(); } catch (e) {} LIVE.peer = null; LIVE.host = false; LIVE.pen = null; LIVE.code = ''; updateLiveUI(); }
  ...
  function updateLiveUI() {
    ...
    if (!LIVE.on) { bar.hidden = true; document.body.classList.remove('live-locked'); document.body.classList.remove('live-guest'); document.body.classList.remove('live-nosave'); return; }
    ...
    const tx = $('liveBarTxt'); if (tx) { const nm = livePenName(); tx.textContent = (LIVE.host ? LIVE.code + ' · ' : '') + '👥 ' + (LIVE.count || 1) + ' · ' + (mine ? t('livePenYou') : (LIVE.pen ? (nm ? '✏️ ' + nm + ' ' + t('livePenHas') : t('livePenOther')) : t('livePenFree'))); }
    const pb = $('livePenBtn'); if (pb) { pb.hidden = false; pb.textContent = mine ? t('liveDropBtn') : t('livePenBtn'); }
  ...
  function fillLiveModal() {
    ...
    if (LIVE.on) {
      const inf = $('liveInfo'); if (inf) inf.textContent = LIVE.host ? (t('liveHosting') + ' ' + LIVE.code + ' · ' + LIVE.conns.length + ' ' + t('liveConnN')) : (t('liveGuestLbl') + ' ' + LIVE.code);
      const su = $('liveShareUrl'); if (su) su.value = liveShareLink();
      const pw = $('livePerms'), pl = $('livePermList');
      if (pw && pl) {
        pw.hidden = !LIVE.host || !LIVE.conns.length;
        if (LIVE.host) {
          pl.innerHTML = LIVE.conns.map((c, i) => '<div class="lvp"><span class="lvp-n">' + esc(c._name || '—') + '</span><button class="lvp-t' + (c._canSave ? ' on' : '') + '" data-i="' + i + '">' + t('liveCanSave') + '</button></div>').join('');
          pl.querySelectorAll('.lvp-t').forEach(b => b.addEventListener('click', () => { const c = LIVE.conns[+b.dataset.i]; if (!c) return; c._canSave = !c._canSave; if (c.open) c.send({ t: 'perm', save: !!c._canSave }); fillLiveModal(); }));
        }
      }
    }
  }

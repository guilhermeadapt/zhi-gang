...
(function () {
  ...
  function makeMemberToken(d) {
    ...
    if (!state.present && !pinned && !d.locked) {
      ...
      g.on('mousedown touchstart', () => {
        ...
        lpTimer2 = setTimeout(() => { const dd = cur().destacados.find(x => x.id === d.id); if (dd) { lpAt = Date.now(); pushUndo(); dd.hideName = false; dd.nameSide = !dd.nameSide; saveProject(); renderTokens(); toast(dd.nameSide ? 'Nome ao lado' : 'Nome embaixo'); } }, 5000);
      });
  ...
  async function deleteScenario(id) {
    ...
    if (state.scenarios.length - remove.size < 1) { toast('É preciso ter ao menos um cenário'); return; }

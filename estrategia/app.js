...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  let gradeMode = 'pt';   // 'pt' = quem está em cada PT (visão de quem MONTA) · 'player' = onde cada um joga
  ...
  function gradeData() {
    ...
    if (gradeMode === 'pt') {
      ...
      PT_IDS.forEach(pid => {
        ...
        if (ids.length) secs.push({ pt: pid, linhas: ids.map(id => ({ p: byId.get(id), cels: state.jogos.map(j => cel(j, id, pid)) })) });
      });
    } else {
      const cel = (j, id) => { const e = j.escalacao[id]; return (e && e.pt) ? (e.reserva ? e.pt + '·r' : e.pt) : null; };
      const linhaDe = id => ({ p: byId.get(id), cels: state.jogos.map(j => cel(j, id)) });
      PT_IDS.forEach(pid => {
  ...
  function renderGrade() {
    ...
    const d = gradeData();
    let h = '<table class="grade-tb"><thead><tr><th>' + esc(gradeMode === 'pt' ? t('gradeColPt') : t('player')) + '</th>';
    d.jogos.forEach(n => { h += '<th>' + esc(n) + '</th>'; });
    h += '</tr></thead><tbody>';
    d.secs.forEach(s => {
      h += '<tr class="g-sec"><td colspan="' + (d.jogos.length + 1) + '">' + esc(s.pt) + '</td></tr>';
      s.linhas.forEach(l => {
        h += '<tr><td class="g-nm" style="--rc:' + roleColor(l.p.funcao) + '">' + esc(l.p.nome) + '</td>';
        l.cels.forEach((c, i) => {
  ...
  async function gradePNG() {
    const d = gradeData();
    const NW = 200, CW = 96, RH = 24, HH = 32, PAD = 18, dpr = 2;
    ...
    g.fillText((gradeMode === 'pt' ? t('gradeColPt') : t('player')).toUpperCase(), PAD + 10, y + HH / 2);
    ...
    d.secs.forEach(s => {
      ...
      s.linhas.forEach(l => {
        g.fillStyle = roleColor(l.p.funcao); g.fillRect(PAD, y, 3, RH);
        g.fillStyle = '#e8ecf4'; g.font = '12px system-ui,sans-serif';
        let nm = l.p.nome; while (g.measureText(nm).width > NW - 20 && nm.length > 3) nm = nm.slice(0, -2) + '…';
  ...
  function syncGradeSwap() { const b = $('gradeSwap'); if (b) b.textContent = gradeMode === 'pt' ? t('gradeByPlayer') : t('gradeByPt'); }
  ...
  function wireEvents() {
    ...
    { const gm = $('gradeModal'), gc = $('gradeClose'), gp = $('gradePng'), gs = $('gradeSwap');
      ...
      if (gs) gs.addEventListener('click', () => { gradeMode = gradeMode === 'pt' ? 'player' : 'pt'; renderGrade(); syncGradeSwap(); });
      syncGradeSwap(); }

...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  function renderPlayerChips() {
    ...
    players.forEach(p => {
      ...
      const rep = p.replaceOf ? '<span class="pl-rep" title="' + esc(t('repOf') + ' ' + p.replaceOf) + '">⇄ ' + esc(p.replaceOf) + '</span>' : '';
  ...
  function openBdPop(id, anchor) {
    ...
    html += '</div>';
    // Replace COM ALVO: escolhe qual titular este jogador substitui (fica no chip)
    const titulares = rosterDraft.filter(x => x.pt && !x.reserva && !x.ausente && x.id !== p.id)

...
<section class="guide-sec"><h3>1 · A ideia — Fases &amp; Cenários</h3><p>Cada <b>cenário</b> é uma "foto" do mapa (posições, desenhos, objetivos up, vida). Você monta uma sequência de fases (ex.: <i>Abertura → Meio → Boss</i>) e navega por elas na régua embaixo (botão <b>Fases</b>). Apresentar roda essa sequência em tela cheia.</p></section>
...
<section class="guide-sec highlight"><h3>5 · Foco / spotlight <span class="tagimp">pouco óbvio</span></h3><p>Ferramenta de <b>alvo</b> na barra esquerda: <b>arraste</b> pra destacar uma área — o resto escurece. <b>Clique curto</b> sobre o foco pra limpar.</p></section>
...
<section class="guide-sec"><h3>1 · La idea — Fases y Escenarios</h3><p>Cada <b>escenario</b> es una "foto" del mapa (posiciones, dibujos, objetivos activos, vida). Armas una secuencia de fases (ej.: <i>Apertura → Medio → Boss</i>) y navegas por ellas en la barra de abajo (botón <b>Fases</b>). Presentar reproduce esa secuencia en pantalla completa.</p></section>
...
<section class="guide-sec highlight"><h3>5 · Foco / spotlight <span class="tagimp">poco obvio</span></h3><p>Herramienta de <b>objetivo</b> en la barra izquierda: <b>arrastra</b> para destacar un área — el resto se oscurece. <b>Toque corto</b> sobre el foco para limpiar.</p></section>
...
<section class="guide-sec"><h3>1 · The idea — Phases &amp; Scenarios</h3><p>Each <b>scenario</b> is a "snapshot" of the map (positions, drawings, active objectives, HP). You build a sequence of phases (e.g. <i>Opening → Mid → Boss</i>) and step through them on the bottom rail (<b>Phases</b> button). Present plays that sequence full-screen.</p></section>
...
<section class="guide-sec highlight"><h3>5 · Focus / spotlight <span class="tagimp">not obvious</span></h3><p><b>Target</b> tool on the left bar: <b>drag</b> to highlight an area — the rest dims. <b>Short tap</b> on the focus to clear it.</p></section>
...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  function renderFocus(previewRect) {
    ...
    if (valid.length) {
      ...
      valid.forEach(f => { const hx = f.x * W, hy = f.y * H, hw = f.w * W, hh = f.h * H, r = Math.min(hw, hh) * 0.05 + 6; focusLayer.add(new Konva.Rect({ x: hx, y: hy, width: hw, height: hh, cornerRadius: r, fill: '#000', globalCompositeOperation: 'destination-out', listening: false })); });
      valid.forEach(f => { const hx = f.x * W, hy = f.y * H, hw = f.w * W, hh = f.h * H, r = Math.min(hw, hh) * 0.05 + 6; focusLayer.add(new Konva.Rect({ x: hx, y: hy, width: hw, height: hh, cornerRadius: r, stroke: 'rgba(240,198,107,0.55)', strokeWidth: 1.5, listening: false, shadowColor: '#f0c66b', shadowBlur: 14, shadowOpacity: 0.35 })); });
      // botão ✕ pra excluir cada foco (só edição, não no preview do arrasto)
      if (!state.present && !previewRect) list.forEach((f, idx) => {
        if (!(f.w > 0.002 && f.h > 0.002)) return;
        const br = Math.max(8, R * 0.62), bg = new Konva.Group({ x: (f.x + f.w) * W, y: f.y * H });
        bg.add(new Konva.Circle({ radius: br, fill: '#E25B52', stroke: '#2a0c0c', strokeWidth: 1.2, shadowColor: '#000', shadowBlur: 3, shadowOpacity: 0.6 }));
        bg.add(new Konva.Text({ text: '✕', fontFamily: 'Barlow, sans-serif', fontStyle: '700', fontSize: br * 1.1, fill: '#fff', align: 'center', verticalAlign: 'middle', width: br * 2, height: br * 2, offsetX: br, offsetY: br }));
        bg.on('click tap', e => { e.cancelBubble = true; pushUndo(); focusList().splice(idx, 1); renderFocus(); saveProject(); });
        bg.on('mouseenter', () => stage.container().style.cursor = 'pointer'); bg.on('mouseleave', () => stage.container().style.cursor = toolCursor());
        focusLayer.add(bg);
      });
    }
  ...
  function makeSceneCard(s, num, isVar) {
    ...
    card.innerHTML = '<div class="sc-top"><span class="sc-idx' + (isVar ? ' sc-idx-var' : '') + '">' + num + '</span><span class="sc-name">' + esc(s.nome || 'Cenário') + '</span></div>' + meta + '<button class="sc-del" title="Excluir" aria-label="Excluir">✕</button>';

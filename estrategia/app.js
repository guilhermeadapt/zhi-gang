...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
        ...
        l.cels.forEach((c, i) => {
          ...
          if (c && typeof c === 'object') {
            ...
            if (c.tor) {   // marcador de torre, encostado à direita da célula
              ...
              g.fillStyle = '#0b0e14'; g.font = 'bold 9px system-ui,sans-serif';
  ...
  function syncGradeSwap() { const b = $('gradeSwap'); if (b) b.textContent = gradeMode === 'print' ? t('gradeBySheet') : (gradeMode === 'sheet' ? t('gradeByPt') : (gradeMode === 'pt' ? t('gradeByPlayer') : t('gradeByPrint'))); }
  ...
  function wireEvents() {
    ...
    { const gm = $('gradeModal'), gc = $('gradeClose'), gp = $('gradePng'), gs = $('gradeSwap');
      ...
      if (gs) gs.addEventListener('click', () => { gradeMode = gradeMode === 'print' ? 'sheet' : (gradeMode === 'sheet' ? 'pt' : (gradeMode === 'pt' ? 'player' : 'print')); renderGrade(); syncGradeSwap(); });
      syncGradeSwap(); }

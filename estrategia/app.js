...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function renderPlayerChips() {
    ...
    players.forEach(p => {
      ...
      chip.innerHTML = classIcoHTML(p.funcao) + '<span class="pl-nm" title="' + esc(p.nome) + '">' + esc(nomeExib(p, 20)) + sabs + rep + '</span><span class="pl-pt">' + esc(where) + '</span><span class="pl-role">' + (placed ? t('onMap') : p.funcao) + '</span>';
      ...
      const grupos = (CFG.grupos || []).map(g => ({
        ...
        pts: (g.pts || []).map(pid => {
          ...
          return {
            ...
            membros: ids.map(id => ({
              p: byId.get(id), tor: j.escalacao[id].torre,
  ...
  function renderPrint(body) {
    ...
    jogos.forEach((j, ji) => {
      ...
      h += '<table class="pr-tb"><thead><tr><th class="pr-j"><span class="pr-jn">' + esc(j.nome) + '</span>'
        + '<span class="pr-obs" data-obs="' + ji + '" title="' + esc(t('gradeObsTip')) + '">' + (j.nota ? esc(j.nota) : '<i>+ ' + esc(t('gradeObs')) + '</i>') + '</span></th>';
      ...
      for (let i = 0; i < j.alt; i++) {
        ...
        j.grupos.forEach(g => g.pts.forEach((x, k) => {
          ...
          h += '<td' + (k === 0 ? ' class="pr-gsep' + (m && m.novo ? ' g-ch' : '') + '"' : (m && m.novo ? ' class="g-ch"' : ''))
            ...
            + (m ? '<span class="g-nome" style="border-left:3px solid ' + roleColor(m.p.funcao) + '">' + esc(nomeExib(m.p, 14)) + '</span>'
  ...
  async function printPNG() {
    const jogos = printData();
    const NW = 190, CW = 150, RH = 22, HH = 30, GAP = 16, PAD = 18, dpr = 2;   // NW cabe nome do jogo + obs
    ...
    jogos.forEach(j => {
      ...
      if (j.nota) {
        g.fillStyle = '#8b93a7'; g.font = 'italic 10px system-ui,sans-serif';
        const wj = g.measureText(String(j.nome).toUpperCase()).width;
        let nt = j.nota; while (g.measureText(nt).width > NW - wj - 14 && nt.length > 3) nt = nt.slice(0, -2) + '…';
        g.fillText(nt, PAD + 12 + wj, y + HH / 2);
      }

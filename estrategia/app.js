...
<section class="guide-sec highlight"><h3>3 · Board (armado de PTs) <span class="tagimp">poco obvio</span></h3><ul><li>Con roster guardado, el modal <b>abre directo en el Board</b>. Arrastra players a PTs, reservas de PT o generales. Las columnas ruedan por separado.</li><li>Herramientas laterales: <b>⚙ Rellenar</b>, <b>🧹 Vaciar</b>, <b>🗂 Clase</b>, <b>🔗 Copiar</b> (enlace solo del board, sin escenas).</li><li><b>Replace con objetivo:</b> clic en un reserva → "⇄ Replace de" lista los titulares de SU PT. El sello aparece en ambos lados.</li><li><b>↔ Otras partidas:</b> con varias partidas en la guerra, el popover del jugador (y el <b>↔</b> del encabezado de la PT) lleva la posición — PT, reserva, función y torre — a las partidas marcadas. Ctrl+Z lo deshace.</li><li><b>Cambio en 1 clic:</b> en el sello ⇄ (pestaña Players) o en el menú del titular en el mapa — el reserva toma el lugar y la POSICIÓN; clic de nuevo lo deshace.</li><li><b>Tour del Board:</b> el botón <b>🎓</b> (junto a 🔗 Copiar) abre un tour guiado dentro del Board: partidas de la guerra, alineación de cada partida, Cuadro y el popover del jugador. Se sale con Esc.</li><li>Cerrar con cambios pendientes <b>pregunta si guardar</b>.</li></ul></section>
...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function sanitizeJogo(j) {
    ...
    return { id: (j && j.id) || uid(), nome: String((j && j.nome) || 'G1').slice(0, 24), nota: (j && typeof j.nota === 'string') ? j.nota.slice(0, 120) : '', escalacao: esc };
  }
  ...
  function printData() {
    ...
    const byId = new Map(state.roster.map(p => [p.id, p]));
    const ord = (a, b) => FO(byId.get(a).funcao) - FO(byId.get(b).funcao) || byId.get(a).nome.localeCompare(byId.get(b).nome);
    return state.jogos.map((j, ji) => {
      const antes = ji > 0 ? state.jogos[ji - 1].escalacao : null;
      ...
      return { nome: j.nome, nota: j.nota || '', grupos, alt: Math.max(alt, 1) };
    });
  }
  ...
  function openGrade() {
    ...
    if (body && !body.dataset.wired) {
      ...
      body.addEventListener('click', e => {
        ...
        const j = state.jogos[+c.dataset.obs]; if (!j) return;
  ...
  async function autoSyncRoster(loud) {
    ...
    let parsed; try { parsed = await fetchRaidHelper(id, state.rhEvent); } catch (e) { if (loud) toast('⚠️ ' + t('rhFail')); return; }
  ...
  function dupGamesHTML(pids) {
    ...
    alvos.forEach(j => { h += '<label class="bp-g"><input type="checkbox" data-g="' + esc(j.id) + '" checked><span>' + esc(j.nome || j.id) + '</span></label>'; });
  ...
  function wireEvents() {
    ...
    { const rl = $('rhLink'), rs = $('rhSync'); if (rs) rs.addEventListener('click', async () => {
      ...
      let parsed; try { parsed = await fetchRaidHelper(id, v); } catch (e) { parseMsg.className = 'parse-msg err'; parseMsg.textContent = t('rhFail'); return; }

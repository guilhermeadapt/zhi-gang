...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in <b>scenario 1</b> and use <b>Duplicate</b> for the next phase.</li><li>Hide names on players that don't need highlighting.</li><li>Use <b>Focus</b> + <b>Present</b> to guide the call.</li></ul></section>` }
  ...
  function init() {
    ...
    try { if (localStorage.getItem('gp-tools-mini') === '1') drawTools.classList.add('mini'); if (localStorage.getItem('gp-side-min') === '1' && !isMobile()) { document.body.classList.add('side-min'); const sm = $('sideMin'); if (sm) sm.textContent = '«'; } } catch (e) {}
  ...
  function applySceneView(s) { if (!s || !s.view) return; const sc = Math.max(1, Math.min(4, s.view.scale || 1)); stage.scale({ x: sc, y: sc }); const x = W / 2 - (s.view.cxf || 0.5) * W * sc, y = H / 2 - (s.view.cyf || 0.5) * H * sc; stage.position(clampPos(x, y, sc)); stage.batchDraw(); updateStageDrag(); }
  function toggleSceneView() { const s = cur(); if (!s) return; if (s.view) { delete s.view; toast(t('zoomCleared')); } else { const sc = stage.scaleX() || 1; s.view = { scale: sc, cxf: clamp01((W / 2 - stage.x()) / sc / W), cyf: clamp01((H / 2 - stage.y()) / sc / H) }; toast(t('zoomSaved')); } updateZoomSaveBtn(); saveProject(); }
  ...
  function updateMini() { const s = cur(); if (s) miniName.textContent = (curIndex() + 1) + '. ' + (s.nome || 'Cenário') + (s.tempo ? ' · ⏱ ' + s.tempo : '') + (s.condicao ? ' · ' + s.condicao : ''); updateSceneHud(); }
  // título grande do cenário + "timer" (tempo de jogo) no topo do mapa
  function updateSceneHud() { const s = cur(); const tt = $('shTitle'), tm = $('shTime'); if (!tt || !tm) return; if (!s) { tt.textContent = ''; tm.hidden = true; return; } tt.textContent = (s.nome || 'Cenário') + (s.condicao ? ' · ' + s.condicao : ''); if (s.tempo) { tm.hidden = false; tm.textContent = '⏱ ' + s.tempo; } else tm.hidden = true; }
  ...
  function bdChip(p) {
    ...
    const rep = p.replaceOf ? '<span class="c-rep has-of" title="' + esc(t('repOf') + ' ' + p.replaceOf) + '">⇄ ' + esc(p.replaceOf.length > 8 ? p.replaceOf.slice(0, 7) + '…' : p.replaceOf) + '</span>'
  ...
  function wireEvents() {
    ...
    rosterSave.addEventListener('click', () => { state.roster = rosterDraft.map(p => Object.assign({}, p)); cleanupReplaces(state.roster); const moved = reconcileMemberSlots(); saveProject(); renderSidebar(); renderTokens(); saveMsg.className = 'parse-msg ok'; saveMsg.textContent = 'Roster salvo ✓' + (moved ? ' · ' + moved + ' posição(ões) herdada(s)' : ''); setTimeout(closeRoster, 500); });

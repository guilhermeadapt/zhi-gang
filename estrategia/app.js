...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  const LIVE = { peer: null, conns: [], host: false, code: '', myId: '', pen: null, on: false, applying: false, txT: null };
  ...
  function liveOnData(m, conn) {
    ...
    else if (m.t === 'who') { const prev = LIVE.count; LIVE.count = m.n; if (LIVE.host) liveRelay(m, conn); if (prev && m.n > prev) toast('👥 ' + t('liveJoinedN') + ' (' + m.n + ')'); else if (prev && m.n < prev) toast('👥 ' + t('liveLeftN') + ' (' + m.n + ')'); updateLiveUI(); }
    else if (m.t === 'hi' && LIVE.host) conn.send({ t: 'init', data: projectData(), pen: LIVE.pen, n: LIVE.conns.length + 1 });
  }
  function liveHost() {
    ...
    p.on('open', id => { LIVE.on = true; LIVE.myId = id; LIVE.pen = id; LIVE.count = 1; clearTimeout(LIVE.connT); updateLiveUI(); fillLiveModal(); });
    p.on('connection', c => {
      c.on('open', () => { LIVE.conns.push(c); LIVE.count = LIVE.conns.length + 1; c.send({ t: 'init', data: projectData(), pen: LIVE.pen, n: LIVE.count }); liveRelay({ t: 'who', n: LIVE.count }, c); toast('👥 ' + t('liveJoinedN') + ' (' + LIVE.count + ')'); updateLiveUI(); fillLiveModal(); });
      c.on('data', m => liveOnData(m, c));
      c.on('close', () => { LIVE.conns = LIVE.conns.filter(x => x !== c); LIVE.count = LIVE.conns.length + 1; liveRelay({ t: 'who', n: LIVE.count }, null); if (LIVE.pen === c.peer) liveSetPen(LIVE.myId); toast('👥 ' + t('liveLeftN') + ' (' + LIVE.count + ')'); updateLiveUI(); fillLiveModal(); });
    });
  ...
  function liveJoin(code) {
    ...
    p.on('open', myId => {
      ...
      c.on('open', () => {
        ...
        LIVE.on = true; LIVE.gotInit = false; clearTimeout(LIVE.connT); updateWarsBtn(); updateLiveUI(); fillLiveModal(); toast(t('liveJoined'));
  ...
  function liveTeardown() { LIVE.on = false; clearTimeout(LIVE.txT); clearTimeout(LIVE.connT); LIVE.count = 0; LIVE.conns.forEach(c => { try { c.close(); } catch (e) {} }); LIVE.conns = []; try { if (LIVE.peer) LIVE.peer.destroy(); } catch (e) {} LIVE.peer = null; LIVE.host = false; LIVE.pen = null; LIVE.code = ''; updateLiveUI(); }
  ...
  function updateLiveUI() {
    const bar = $('liveBar'), ov = $('liveLockOv'), lb = $('liveBtn'); if (!bar || !ov) return;
    if (lb) lb.classList.toggle('primary', LIVE.on);
    if (!LIVE.on) { bar.hidden = true; ov.hidden = true; document.body.classList.remove('live-locked'); return; }
    const mine = livePenMine();
    bar.hidden = false; ov.hidden = mine; document.body.classList.toggle('live-locked', !mine);
    const tx = $('liveBarTxt'); if (tx) tx.textContent = (LIVE.host ? LIVE.code + ' · ' : '') + '👥 ' + (LIVE.count || 1) + ' · ' + (mine ? t('livePenYou') : (LIVE.pen ? t('livePenOther') : t('livePenFree')));
  ...
  function wireEvents() {
    ...
    { const lb = $('liveBtn'), lm = $('liveModal');
      if (lb && lm) {
        lb.addEventListener('click', () => { fillLiveModal(); lm.hidden = false; });

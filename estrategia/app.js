...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function init() {
    ...
    renderSidebar(); renderJogos(); renderRail(); loadScenarioIntoUI(); syncCanvasFill(cur()); fit(); applySceneView(cur()); updateZoomSaveBtn(); wireEvents(); (async () => { await maybeLoadShared(); await maybeLoadScenes(); await maybeLoadObj(); await maybeLoadBoard(); await maybeLoadPlan(); await maybeLoadCommand(); autoSyncRoster(maybeLoadRh()); maybeTour(); })();
  ...
  // ---- persistência ----
  function saveProject() { stashJogoAtual(); try { localStorage.setItem(CFG.projectKey, JSON.stringify({ v: 6, currentId: state.currentId, scenarios: state.scenarios, roster: state.roster, ptDesc: state.ptDesc, ptIcon: state.ptIcon, objetivoPosGlobal: state.objetivoPosGlobal, gates: state.gates, side: state.side, showNames: state.showNames, rhEvent: state.rhEvent || '', rhEvent2: state.rhEvent2 || '', jogos: state.jogos, jogoAtual: state.jogoAtual })); } catch (e) {} if (LIVE.on && !LIVE.applying && livePenMine()) liveScheduleTx(); }
  function loadProject() { try { const raw = localStorage.getItem(CFG.projectKey); if (raw) { const d = JSON.parse(raw); if (Array.isArray(d.scenarios) && d.scenarios.length) { state.scenarios = d.scenarios.map(sanitizeScenario); state.currentId = d.currentId && state.scenarios.some(s => s.id === d.currentId) ? d.currentId : state.scenarios[0].id; if (Array.isArray(d.roster)) state.roster = d.roster.map(sanitizePlayer); state.objetivoPosGlobal = sanitizePosMap(d.objetivoPosGlobal); state.gates = sanitizePosMap(d.gates); state.side = (d.side === 'blue' || d.side === 'red') ? d.side : null; state.ptDesc = sanitizeDesc(d.ptDesc); state.ptIcon = sanitizeDesc(d.ptIcon); if (typeof d.showNames === 'boolean') state.showNames = d.showNames; if (typeof d.rhEvent === 'string') state.rhEvent = d.rhEvent; if (typeof d.rhEvent2 === 'string') state.rhEvent2 = d.rhEvent2; if (Array.isArray(d.jogos)) { state.jogos = d.jogos.map(sanitizeJogo); state.jogoAtual = state.jogos.some(j => j.id === d.jogoAtual) ? d.jogoAtual : (state.jogos[0] ? state.jogos[0].id : null); } return; } } } catch (e) {} const s = newScenario({ fase: 'Start', nome: 'Start (30m)' }); state.scenarios = [s]; state.currentId = s.id; }

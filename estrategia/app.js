...
<section class="guide-sec tips"><h3>Tips</h3><ul><li>Build the base in scene 1 and <b>Duplicate</b> per phase.</li><li>Save the <b>per-scene zoom</b> (📍) before presenting.</li><li><b>Auto-focus</b> + <b>Present</b> guide attention.</li><li>The <b>What's new</b> tab shows every change by version.</li></ul></section>` }
  ...
  function renderPrint(body) {
    ...
    let h = '';
    jogos.forEach((j, ji) => {
      h += '<div class="pr-jogo"><div class="pr-head"><b>' + esc(j.nome) + '</b>'
        + '<span class="pr-obs" data-obs="' + ji + '" title="' + esc(t('gradeObsTip')) + '">' + (j.nota ? esc(j.nota) : '<i>+ ' + esc(t('gradeObs')) + '</i>') + '</span></div>';
      h += '<table class="pr-tb"><thead><tr><th class="pr-n"></th>';
  ...
  function renderGrade() {
    ...
    { const hint = document.querySelector('.grade-hint'); if (hint) hint.textContent = t(gradeMode === 'print' ? 'gradeHintPrint' : 'gradeHint'); }
  ...
  async function printPNG() {
    const jogos = printData();
    const NW = 34, CW = 150, RH = 22, HH = 26, TT = 26, GAP = 14, PAD = 18, dpr = 2;
    const cols = jogos[0] ? jogos[0].grupos.reduce((a, g) => a + g.pts.length, 0) : 6;
    const W = PAD * 2 + NW + CW * cols;
    let H = PAD * 2 + 16;
    jogos.forEach(j => { H += TT + HH + RH * j.alt + GAP; });
    ...
    let y = PAD;
    jogos.forEach(j => {
      g.fillStyle = '#d9a441'; g.font = 'bold 13px system-ui,sans-serif';
      g.fillText(String(j.nome).toUpperCase(), PAD, y + TT / 2);
      if (j.nota) {
        g.fillStyle = '#8b93a7'; g.font = 'italic 11px system-ui,sans-serif';
        g.fillText(j.nota, PAD + g.measureText(String(j.nome).toUpperCase()).width + 26, y + TT / 2);
      }
      y += TT;
      let cx = PAD + NW;
      ...
      for (let i = 0; i < j.alt; i++) {
        ...
        g.fillText(String(i + 1), PAD + 11, y + RH / 2);
        cx = PAD + NW;
    ...
    g.fillStyle = '#5a6274'; g.font = '10px system-ui,sans-serif';
    g.fillText('Game Plan · ' + jogos.length + ' ' + (jogos.length === 1 ? 'jogo' : 'jogos'), PAD, H - PAD + 2);
    return cv;
  }
